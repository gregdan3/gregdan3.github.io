...
<a
	href={`/blog/${slug}`}
	class="not-prose relative h-56 bg-table dark:bg-table-dark hover:bg-ends hover:dark:bg-ends-dark transition-all ease-in-out duration-300 rounded-lg shadow-md border border-accent dark:border-ctrim-dark"
>
	{#if image}
		<img
			src={image}
			alt="blog-banner"
			class="absolute top-0 left-0 w-full h-2/3 rounded-t-lg object-cover border-b border-accent dark:border-ctrim-dark"
		/>
	{:else}
		<div
			class="absolute top-0 left-0 w-full h-2/3 bg-gradient-to-br from-table to-accent dark:from-table-dark dark:to-trim-dark rounded-t-lg border-b border-accent dark:border-ctrim-dark"
		></div>
	{/if}

	<div class="absolute top-4 left-4">
		{#if tags}
			<div class="flex flex-wrap space-x-0.5 m-0.5">
				{#each tags as tag}
					<span
